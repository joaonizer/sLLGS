...
N = 16000;       % numero de passos
tempo_total=8e-9;% Tempo total de simula̤̣o
alpha=0.05;%;0.054;
T=400;        % Kelvin
Ms=800e3;   % A/m
kbT=kb*T;   % J
...
J_shm=0*1e12; % Spin Hall current density (A/m2)
...
hd=hc;
m_no_re=zeros(N+1,3,part_n);
m_module=zeros(N+1,part_n);
for i = 1 :N
    %progress=i/N*100;
...
plot(t(2:10:end),log10(abs(1-abs(m_module(2:10:end,:)))));
xlabel('Tempo (ns)');
ylabel('log_{10}(|1-|m||)')
