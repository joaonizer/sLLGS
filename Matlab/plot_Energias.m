...
global Ms Ns q time_step alpha gammamu0 K1 T;
mu0=4*pi*1e-7;
%% Energia Térmica
...
% onde Nxx, Nxy etc. são as componentes do tensor desmagnetização
hd(:,1)=Ns(1,1)*m(:,1)+Ns(1,2)*m(:,2)+Ns(1,3)*m(:,3);
hd(:,2)=Ns(2,1)*m(:,1)+Ns(2,2)*m(:,2)+Ns(2,3)*m(:,3);
hd(:,3)=Ns(3,1)*m(:,1)+Ns(3,2)*m(:,2)+Ns(3,3)*m(:,3);
